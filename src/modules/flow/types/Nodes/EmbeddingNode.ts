  ...
  return {
    ...
    process: async ({ node, getInputValue }) => {
      ...
      try {
        ...
        const inputString = typeof inputValue === 'string' ? inputValue : JSON.stringify(inputValue);
        ...
        console.log(`Executing Embedding node ${n.id} with ${chunks.length} chunks`);
        
        // Get API key from config
        let API_KEY = "";
        if (n.getConfigParameter) {
          API_KEY = n.getConfigParameter("Google API Key")?.paramValue as string || "";
        }
        
        if (!API_KEY) {
          throw new Error("Google API Key not found. Please configure the Google API Key in node settings.");
        }
        
        // Get model from node value or config
        const model = n.getConfigParameter ? 
          (n.getConfigParameter("Model")?.paramValue as string || n.nodeValue?.toString() || "models/gemini-embedding-001") :
          (n.nodeValue?.toString() || "models/gemini-embedding-001");
        
        console.log(`Making API request to Google Gemini embeddings with model: ${model}`);
        ...
        const embeddings = data.embeddings.map((embedding: { values: number[] }) => (
          embedding.values // Google returns embedding vectors in .values
        ));
        
        console.log(`Embedding node ${n.id} processed ${embeddings.length} chunks in batch`);
        
        // Return the results - for single string input, return just the embedding vector
        return {
          // Socket id 2 is for Embeddings output
          [n.id * 100 + 2]: isSingleString && embeddings.length === 1 
            ? JSON.stringify(embeddings[0], null, 2) 
            : JSON.stringify({ embeddings }, null, 2),
          // Socket id 3 is for Status
          [n.id * 100 + 3]: `Success: Generated embeddings for ${chunks.length} ${isSingleString ? 'text input' : 'chunks'} using Google Gemini`,
        };
