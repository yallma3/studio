...
import { ConsoleEvent, ParsedProjectPlan } from '../types/Types'
...
import { ExecutionStep, StepExecutionResult, SequentialExecutionOptions } from '../types/Types';



export async function executeTasksSequentially(options: SequentialExecutionOptions): Promise<StepExecutionResult[]> {
  const {
    nodeRegistry,
    workspaceData,
    steps,
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    // Find workflow
    const workflow = workspaceData.workflows.find(w => w.id === step.workflow || w.name === step.workflow);
    ...
    // Notify step start
    console.log(`\n🚀 Running step ${stepNumber}/${steps.length}: ${task.name} with agent: ${agent?.name || workflow?.name || 'Unknown'}`);
    ...
    try {
      ...
      if(agent){
        ...
        const runtime = new AgentRuntime(agent, task, groqRunner, dependencyContext);
        ...
        if(workflowData){
          const reattachedNodes = reattachNodeProcessors(workflowData.canvasState.nodes);
          workflowData.canvasState.nodes = reattachedNodes;
          console.log("🔄 Running workflow:", "Nodes", workflowData.canvasState.nodes, "Connections", workflowData.canvasState.connections);
          const runtime = createFlowRuntime(workflowData.canvasState.nodes, workflowData.canvasState.connections);
...
export function convertToSequentialSteps(parsedPlan: ParsedProjectPlan): ExecutionStep[] {
...
export class GroqChatRunner implements ChatRunner {
    ...
    async run(prompt: string, systemPrompt: string = ""): Promise<string | null> {
      ...
      try {
        ...
        if (result && typeof result === "object" && "3" in result && "4" in result) {
          const responseText = result?.[3]?.toString() || "";
          const tokenCount = Number(result?.[4]) || 0;
    
          console.log("📝 Response:", responseText);
          console.log("🔢 Tokens:", tokenCount);
    
          const isError = responseText.toLowerCase().startsWith("error:");
          this.emitEvent(isError ? "error" : "success", `Groq LLM ${isError ? "failed" : "completed"}`);
    ...
    private emitEvent(type: "info" | "warning" | "error" | "success", message: string) {
      this.addEvent({
...
export async function getWorkflow(flowId: string): Promise<WorkflowFile | null> {
  try {
 ...
 */
function buildDependencyContext(step: ExecutionStep, results: StepExecutionResult[]): string {
  ...
  const dependencyResults = step.dependsOn
    .map(stepNumber => {
      ...
      if (!dependencyResult) {
        console.warn(`⚠️ Dependency step ${stepNumber} not found for step ${step.step}`);
        return null;
      }
      
      if (!dependencyResult.success) {
        console.warn(`⚠️ Dependency step ${stepNumber} failed for step ${step.step}`);
        return null;
      }
  ...
  console.log(`📋 Built dependency context for step ${step.step}:`, contextLines.join("\n"));
 ...
 */
function validateDependencies(step: ExecutionStep, results: StepExecutionResult[]): { isValid: boolean; error?: string } {
  if (!step.dependsOn || step.dependsOn.length === 0) {
    return { isValid: true };
  }
  ...
  for (const depStepNumber of step.dependsOn) {
    ...
    if (!depResult.success) {
      return { 
        isValid: false, 
        error: `Dependency step ${depStepNumber} failed: ${depResult.error || 'Unknown error'}` 
      };
    }
  }
